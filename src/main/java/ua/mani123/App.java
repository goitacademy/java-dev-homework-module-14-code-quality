...
public class App {
    ...
    public void runApp() {
        ...
        while (true) {
            if (checkWinner()) {
                ...
                playerTurn(); // You can change who first start game
                computerTurn();
                printPanel(); // Game panel
            }
        }
    }
    ...
    public void playerTurn() {
        while (true) {
            ...
            if (input > 0 && input < 10) {
                if (box[input - 1] == 'X' || box[input - 1] == 'O') logger.error("That one is already in use. Enter another.");
                else {
                    box[input - 1] = 'X';
                    return;
                }
